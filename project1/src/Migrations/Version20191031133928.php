 ...
 */
final class Version20191031133928 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('DROP VIEW public.weekly_activity');
        $this->addSql('CREATE SEQUENCE atp_id_seq INCREMENT BY 1 MINVALUE 1 START 1');
        $this->addSql('CREATE TABLE atp (id INT NOT NULL, values TEXT NOT NULL, phases TEXT NOT NULL, PRIMARY KEY(id))');
  ...
  SELECT (round(avg(foo.vertical_oscillation)::numeric, 2) || \' \'::text) || array_agg(round(foo.vertical_oscillation::numeric, 2))::text AS vert_osc, 
   ...
   ORDER BY date_trunc(\'week\'::text, foo.start_time_local + \'00:00:00\'::interval)::date DESC;');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('DROP SEQUENCE atp_id_seq CASCADE');
        $this->addSql('DROP TABLE atp');
        $this->addSql('DROP TABLE weekly_activity');
        $this->addSql('ALTER TABLE garmin_activity_details ALTER description TYPE VARCHAR(255)');
        $this->addSql('ALTER TABLE garmin_activity_details ALTER description DROP DEFAULT');
        $this->addSql('ALTER TABLE garmin_activity_details ALTER comments TYPE VARCHAR(255)');
        $this->addSql('ALTER TABLE garmin_activity_details ALTER comments DROP DEFAULT');
    }
}
